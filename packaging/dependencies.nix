...
in
  scope: {
    ...
    mimalloc =
      if lib.versionAtLeast pkgs.mimalloc.version "3.3.2"
      then pkgs.mimalloc
      else
    ...
    sqlite =
      ...
      then pkgs.sqlite
        ...
        pkgs.sqlite.overrideAttrs (prevAttrs: {
          ...
          configureFlags =
            (lib.filter (x: !(lib.hasPrefix "--with-tcl" x)) prevAttrs.configureFlags)
            ++ [
              "--disable-tcl"
            ];
        });
    ...
    libgit2 = pkgs.libgit2.overrideAttrs (attrs: rec {
      version = "1.9.6";
      src = pkgs.fetchFromGitHub {
        owner = "libgit2";
        repo = "libgit2";
